...
const AddPaymentAndMessage = ({ creatorProfile, authUser, onPurchaseStatus, onClose}) => {
  ...
  const handleSubmit = async (e, useCurrentPayment = false) => {
    ...
    try {
      ...
      const response = await axios.post(
        ...
        {
          userId,
          price: creatorProfile.creatorMessagePrice,
          paymentMethodId,
        }
      );
      ...
      <div>
        ...
        {showAddPaymentForm || paymentMethods.length === 0 ? (
          <PaymentForm
              ...
              onPurchaseStatus={onPurchaseStatus}
              onClose={onClose}
          />
        ) : null}
      </div>
...
const PaymentForm = ({ userId, creatorProfile, onPurchaseStatus, loading, setLoading, onClose }) => {
  const stripe = useStripe();
  const elements = useElements();
...
export default AddPaymentAndMessage;
